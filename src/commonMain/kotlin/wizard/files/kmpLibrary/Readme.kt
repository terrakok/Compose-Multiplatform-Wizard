...
class Readme(info: ProjectInfo) : ProjectFile {
    ...
    override val content = buildString {
        ...
        appendLine("")
        appendLine("1) Create a account and a namespace on Sonatype:")
        ...
        appendLine("   `/convention-plugins/src/main/kotlin/convention.publication.gradle.kts`")
        ...
        appendLine("   ```")
        appendLine("4) Add the secrets to `local.properties`:")
        appendLine("   ```")
        ...
        appendLine("   ossrhUsername=[Sonatype token_user]")
        appendLine("   ossrhPassword=[Sonatype token_password]")
        appendLine("   ```")
        appendLine("5) Run `./gradlew :${info.moduleName}:publishAllPublicationsToSonatypeRepository`")
    }
}